...
export default function ClassDetailView({ backHref = "/admin/dashboard", rolePrefix = "/admin" }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <button onClick={() => router.push(backHref)} className="text-xs text-slate-500 hover:text-slate-800 flex items-center gap-1 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {classData.sections.map((section) => {
            ...
            return (
              <button
                key={section.id}
                onClick={() => router.push(`${rolePrefix}/school/classes/${classId}/${section.section_slug || section.id}`)}
