...
export default function SectionDetailView({ rolePrefix = "/admin" }) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <button onClick={() => router.push(`${rolePrefix}/school/classes/${classId}`)} className="text-xs text-slate-500 hover:text-slate-800 flex items-center gap-1 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((student) => {
            ...
            return (
              <button
                key={student.id}
                onClick={() => router.push(`${rolePrefix === "/admin" ? "/admin/students" : "/principal/students"}/${student.id}`)}
