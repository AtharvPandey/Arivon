...
export default function StudentProfileView({ readOnly = false, backHref = "/admin/students" }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => router.push(backHref)} className="text-sm text-slate-600 hover:text-slate-900 flex items-center gap-1">
          <ArrowLeft size={14} /> Back
        </button>
        {!readOnly && student && !editing && (
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 mt-4">
                ...
                <div className="space-y-2">
                  {siblings.map((sib) => (
                    <button
                      key={sib.id}
                      onClick={() => router.push(`/admin/students/${sib.id}`)}
