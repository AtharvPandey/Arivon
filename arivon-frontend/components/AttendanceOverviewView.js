...
function AttendanceOverviewInner({ backHref }) {
  ...
  async function handleStudentSearch(e) {
    ...
    try {
      const results = await apiRequest(
        `/admin/morning-briefing/student-search?school_id=${schoolId}&query=${encodeURIComponent(studentQuery)}`
      );
                ...
                return (
                  <div key={s.section_id} className="px-5 py-4 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <a
                        href={`/admin/attendance?class=${s.class_id}&section=${s.section_id}`}
...
export default function AttendanceOverviewView({ backHref = "/admin" }) {
  return (
    <Suspense fallback={<div className="max-w-5xl mx-auto px-6 py-8 text-sm text-slate-500">Loading...</div>}>
      <AttendanceOverviewInner backHref={backHref} />
    </Suspense>
  );
}
