          ...
          {houses.map((house) => {
            ...
            return (
              <div key={house.id} className="rounded-2xl overflow-hidden shadow-sm border border-slate-200 bg-white">
                ...
                <button
                  onClick={() => !isEditing && router.push(`/admin/school/houses/${house.id}`)}
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between mb-1">
                    <button onClick={() => router.push(`/admin/school/houses/${house.id}`)} className="text-lg font-display font-bold text-slate-900 hover:text-indigo-700">
                      {house.name}
                    </button>
