...
import { ArrowLeft, Upload, CheckCircle2, Circle, IndianRupee, Lock } from "lucide-react";
...
// Same split as the backend: moving to offer_sent or rejected is the
// actual accept/reject DECISION, restricted to Principal/VP. Every
// other transition is administrative pipeline progression, open to
// whoever's already viewing the application.
const DECISION_TRANSITIONS = new Set(["offer_sent", "rejected"]);
const APPROVAL_ROLES = new Set(["principal", "vice_principal"]);
...
export default function ApplicationDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      <button onClick={() => router.push("/admin/admissions")} className="text-sm text-slate-600 hover:text-slate-900 mb-4 flex items-center gap-1">
        ...
        {NEXT_STEPS[app.status]?.length > 0 && (
          <div className="flex flex-wrap gap-2 pt-3 border-t border-slate-100">
            {NEXT_STEPS[app.status].map((step) => {
              const isDecision = DECISION_TRANSITIONS.has(step);
              const canAct = !isDecision || APPROVAL_ROLES.has(role);
              if (isDecision && !canAct) {
                return (
                  <span
                    key={step}
                    title="Only the Principal or Vice Principal can approve or reject an application"
                    className="text-xs font-medium border border-slate-100 text-slate-400 rounded-lg px-3 py-1.5 capitalize flex items-center gap-1 cursor-not-allowed"
                  >
                    <Lock size={11} /> Move to {step.replace("_", " ")}
                  </span>
                );
              }
              return (
      ...
      {enrolledStudent && (
        <div className="bg-white border border-slate-200 rounded-xl p-6 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => router.push(`/admin/students/${enrolledStudent.id}`)}
